...
from scheduler.scheduler import send_reminders_scheduled_on
from scheduler.smsclient import SMSClient
from testdata.test_data import DATA_WINNER_LOGIN_PAGE
from tests.createprojecttests.create_project_data import VALID_DATA
from tests.logintests.login_data import TRIAL_CREDENTIALS, VALID_CREDENTIALS
...
class TestReminderSend(BaseTest):
    ...
    def get_sent_reminder_count(self, all_reminders_page):
        all_reminders_page.click_sent_reminder_tab()
        table_length_before_send = self.driver.find_elements_by_css_selector('tr').__len__()-1
        sms_client = Mock(spec=SMSClient)
        sms_client.send_reminder.return_value=1
        send_reminders_scheduled_on(date.today(), sms_client)
        all_reminders_page.click_sent_reminder_tab()
        table_length_after_send = self.driver.find_elements_by_css_selector('tr').__len__()-1
        return table_length_after_send, table_length_before_send

    @attr("functional_test")
    def test_trial_account_should_not_send_reminder(self):
        dashboard_page = self.login_with(TRIAL_CREDENTIALS)
        create_project_page = self.create_project(dashboard_page)
        reminder_page = self.create_project_to_reminder_page(create_project_page)
        self.create_reminder_for_all_week(reminder_page)
        all_reminders_page = self.active_project_and_go_to_all_reminder_page(reminder_page)
        table_length_after_send, table_length_before_send = self.get_sent_reminder_count(all_reminders_page)
        self.assertEqual(table_length_after_send, table_length_before_send)

    @attr("functional_test")
    def test_paid_account_should_send_reminder(self):
        dashboard_page = self.login_with(VALID_CREDENTIALS)
        create_project_page = self.create_project(dashboard_page)
        reminder_page = self.create_project_to_reminder_page(create_project_page)
        self.create_reminder_for_all_week(reminder_page)
        all_reminders_page = self.active_project_and_go_to_all_reminder_page(reminder_page)
        table_length_after_send, table_length_before_send = self.get_sent_reminder_count(all_reminders_page)
        self.assertEqual(table_length_after_send, table_length_before_send+1)
